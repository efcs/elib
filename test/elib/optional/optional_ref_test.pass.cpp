...
BOOST_AUTO_TEST_SUITE(elib_optional_ref_test_suite)
...
BOOST_AUTO_TEST_CASE(uninit_copy_assign_test)
{
    int x = 1;
    opt const t;
    opt t2(x);
...
BOOST_AUTO_TEST_CASE(init_copy_assign_test)
{
    int x = 1;
    opt const t(x);
    opt t2;
...
BOOST_AUTO_TEST_CASE(uninit_move_assign_test)
{
    int x = 1;
    opt t;
    opt t2(x);
...
BOOST_AUTO_TEST_CASE(init_move_assign_test)
{
    int x = 1;
    opt t(x);
    opt t2;
...
BOOST_AUTO_TEST_CASE(deref_operator_test)
{
    int x = 1;
    opt const t(x);
    BOOST_CHECK(&t.operator*() == &x);
...
BOOST_AUTO_TEST_CASE(non_empty_value_test)
{
    int x = 1;
    opt const t(x);
    BOOST_CHECK(&t.value() == &x);
