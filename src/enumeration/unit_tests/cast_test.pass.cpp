...
TEST_SUITE(enumeration_cast_test_suite)

TEST_CASE(enumeration_cast_bad_enum_cast_coverage_test)
{
...
TEST_CASE(enumeration_enum_cast_enum_to_integral)
{
    int dest = 1;
...
TEST_CASE(enumeration_enum_cast_integral_or_enum_to_enum)
{
    {
        A dest = A::none;
    ...
    }
    {
        ...
        TEST_CHECK_THROW(bad_enum_cast, enum_cast<A>(5));
        TEST_CHECK(enum_cast<A>(B::one) == A::one);
        TEST_CHECK_THROW(bad_enum_cast, enum_cast<A>(static_cast<B>(5)));
    }
    {
        TEST_CHECK(enum_cast<C>(1) == C::one);
        TEST_CHECK_THROW(bad_enum_cast, enum_cast<C>(5));
        TEST_CHECK(enum_cast<C>(B::one) == C::one);
        TEST_CHECK_THROW(bad_enum_cast, enum_cast<C>(static_cast<B>(5)));
    }
}

TEST_CASE(enumeration_enum_cast_enum_to_string)
{
    TEST_CHECK(enum_cast<std::string>(C::none) == "none");
    TEST_CHECK(enum_cast<std::string>(C::two) == "two");
    TEST_CHECK_THROW(bad_enum_cast, enum_cast<std::string>(static_cast<C>(5)));
}

TEST_CASE(enumeration_enum_cast_string_to_enum)
{
    TEST_CHECK(enum_cast<C>("none") == C::none);
    TEST_CHECK(enum_cast<C>("two") == C::two);
    TEST_CHECK_THROW(bad_enum_cast, enum_cast<C>(""));
    TEST_CHECK_THROW(bad_enum_cast, enum_cast<C>("three"));
