...
# define ELIB_PCHECK(lhs, op, rhs) _ELIB_PCHECK(lhs, op,  rhs, __LINE__, __FILE__,  __func__)

#   define _ELIB_PCHECK(lhs, op, rhs, line, file, func) \
  ::elib::elib_pcheck(lhs op rhs, "" # lhs,  "" # op, "" # rhs, lhs, rhs, line,  file, func)
...
namespace elib
{
                   ...
                   int line, const char* file,  const char* func)
  {
    ...
    std::cout <<  file << "::" << line  << ": In function " << func 
              << ": PCHECK FAILED" <<  std::endl;
              
    std::cout << "\t" << lhs << " " << op <<  " " <<  rhs << std::endl;
    
    std::cout <<  "\t" <<  "With " << lhs << " = " << "\"" << lhs_val << "\""
              << std::endl;
              
    std::cout <<  "\t" << "With " <<  rhs << " = " << "\"" << rhs_val << "\""
              << std::endl;
